...
class DIYBezierView : BaseView {
   ...
   override  fun init(context: Context) {
       ...
       mPaint!!.color = parseColor(BEZIER_CIRCLE_COLOR)

        mCirclePaint = Paint()
        mCirclePaint!!.setAntiAlias(true)
        mCirclePaint!!.setStyle(Paint.Style.STROKE)
        mCirclePaint!!.setStrokeWidth(LINE_WIDTH.toFloat())
        mCirclePaint!!.setColor(parseColor(NATIVE_CIRCLE_COLOR))

        mControlPaint = Paint()
        mControlPaint!!.setAntiAlias(true)
        mControlPaint!!.setStyle(Paint.Style.STROKE)
        mControlPaint!!.setStrokeWidth(LINE_WIDTH.toFloat())
        mControlPaint!!.setColor(parseColor(CONTROL_LINE_COLOR))
    ...
    override  fun onDraw(canvas: Canvas) {
        ...
        for (i in 0..3) {
            ...
            if (i == 0) {
                ...
                mControlPath!!.moveTo(
                    mControlPointList!![startIndex - 1].x,
                    mControlPointList!![startIndex - 1].y
                )
            }

            mControlPath!!.lineTo(
                mControlPointList!![startIndex].x,
                mControlPointList!![startIndex].y
            )
            mControlPath!!.lineTo(
                mControlPointList!![startIndex + 1].x,
                mControlPointList!![startIndex + 1].y
        ...
        for (i in mControlPointList!!.indices) {
            ...
            if (mCurSelectPointList.contains(point)) {      // 绘制选中的点
                mControlPaint!!.setColor(parseColor(SEL_POINT_COLOR))
        ...
        if (mStatus == Status.THREE) {
            ...
            for (i in 0 until mCurSelectPointList.size - 1) {
                val p1 = mCurSelectPointList.get(i)
                val p2 = mCurSelectPointList.get(i + 1)
                mControlPaint!!.setColor(parseColor(SEL_POINT_COLOR))
